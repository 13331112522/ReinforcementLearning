...
class ImageProcess():
    ...
    def ColorMat2Binary(self, state):
        ...
        state_gray = cv.cvtColor(state, cv.COLOR_BGR2GRAY)
        # print state_gray.shape
        # cv2.imshow('test2', state_gray)
        # cv2.waitKey(0)

        _, state_binary = cv.threshold(state_gray, 5, 255, cv.THRESH_BINARY)

        state_binarySmall = cv.resize(state_binary, (sWidth, sHeight), interpolation=cv.INTER_AREA)
